"""
Generate FP examples for COT
I have already generated the data for you in /data/patchparser-data.tar.gz if you do not want to run this.

To execute, make sure the code/llm/cfgs/v1_mixtral_cot_generate_fp.yaml is updated with your paths!!!
Install the requirements.txt file.

Then: python3 ./code/llm/cot-generate-fp-example.py v1_mixtral_cot_generate_fp
"""
import sys  # noqa
...
if is_debugger:
    config_file = "v1_mixtral_cot_generate_fp"
    debug_test_amount = 1
...
if __name__ == '__main__':
    ...
    # create a holder for the FP data
    data_fp_examples = pd.DataFrame()

    for idx, row in data_fp.iterrows():
        print(f"\n\n{idx}/{len(data_fp)}")
        ...
        temp_fp = pd.DataFrame(row).T
        temp_fp['cot_example_text'] = response.replace('</s>', '').strip()

        # append back to the data fp examples
        data_fp_examples = pd.concat([data_fp_examples, temp_fp])

    print(
        f"Saving FP COT examples to: {CFG.paths.patchparser}{CFG.data.fp_cot_examples}")

    data_fp_examples = data_fp_examples.reset_index(drop=True)

    data_fp_examples.to_csv(f"{CFG.paths.patchparser}{CFG.data.fp_cot_examples}",
